...
-- เก็บเฉพาะ SHA-256 ของ token ไม่เก็บตัวจริง ถ้าฐานข้อมูลรั่วก็สวมสิทธิ์ผู้ใช้ไม่ได้
...
# CREATE TABLE IF NOT EXISTS ไม่เพิ่มคอลัมน์ให้ตารางที่มีอยู่แล้ว คอลัมน์ที่เพิ่มภายหลังจึงต้องสั่งแยก
# ผู้ใช้เดิมที่ผู้ดูแลสร้างเองได้ status = 'active' ตามค่าเริ่มต้น ส่วนคนที่สมัครเองจะถูกตั้งเป็น 'pending'
MIGRATIONS = """
...
def initialize_database() -> None:
    with connect() as connection:
        connection.execute(SCHEMA + MIGRATIONS + ENABLE_ROW_LEVEL_SECURITY)
